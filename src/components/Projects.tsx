  ...
  return (
    <section id="Projects" style={{ padding:"7rem 0" }}>
        ...
        <div style={{ display:"flex", gap:".5rem", flexWrap:"wrap" }}>
          {FILTERS.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{
              ...
              borderRadius:"4px", background:activeFilter===f?"rgba(79,255,176,.07)":"transparent",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(310px, 1fr))", gap:"1.4rem" }}>
        {filtered.length===0 ? (
          <div style={{ gridColumn:"1/-1", textAlign:"center", padding:"4rem", border:"1px dashed var(--border)", borderRadius:"var(--r-md)", fontFamily:"var(--mono)", fontSize:".82rem", color:"var(--muted)" }}>
            // Aucun projet dans cette catégorie.
          </div>
        ) : filtered.map(project => {
          const tc = typeConfig[(project.type as TypeKey) || "fullstack"] ?? typeConfig.fullstack
          const statutColor = project.statut ? (STATUT_COLOR[project.statut] || "var(--muted)") : null
            ...
            >
              ...
              {project.image && (
                <div style={{ position:"relative", overflow:"hidden", height:200 }}>
                  ...
                  {/* Badges */}
                  <div style={{ position:"absolute", top:10, left:10, display:"flex", gap:".4rem", flexWrap:"wrap" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:".3rem", background:"rgba(7,9,14,.82)", backdropFilter:"blur(6px)", border:`1px solid ${tc.color}40`, borderRadius:"4px", padding:".2rem .6rem", fontFamily:"var(--mono)", fontSize:".62rem", color:tc.color, letterSpacing:".08em", textTransform:"uppercase" }}>
                      {tc.icon} {tc.label}
                    </div>
                    {statutColor && project.statut && (
                      <div style={{ background:"rgba(7,9,14,.82)", backdropFilter:"blur(6px)", border:`1px solid ${statutColor}40`, borderRadius:"4px", padding:".2rem .6rem", fontFamily:"var(--mono)", fontSize:".62rem", color:statutColor, letterSpacing:".06em" }}>
                        {project.statut}
                      </div>
                    )}
                  </div>
              ...
              <div style={{ padding:"1.3rem 1.4rem", flex:1, display:"flex", flexDirection:"column" }}>
                {/* Rôle */}
                {project.role && (
                  <p style={{ fontFamily:"var(--mono)", fontSize:".65rem", color:"var(--muted)", letterSpacing:".06em", marginBottom:".5rem", textTransform:"uppercase" }}>
                    {project.role}
                  </p>
                )}
